...
Three methods all set a reactive `aoi()` polygon:
1. **watershed_group_name** — type-ahead dropdown querying `whse_basemapping.fwa_watershed_groups_poly`
2. **Click-to-delineate** — click basemap → snap via `fresh::frs_point_snap()` → delineate via `fresh::frs_network()`
...
- `app.R` as golem launcher (`pkgload::load_all()` + `run_app()`)
