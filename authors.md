# Authors and Citation

## Authors

- **Al Irvine**. Author, maintainer.

- **New Graph Environment Ltd.**. Copyright holder.

## Citation

Source:
[`DESCRIPTION`](https://github.com/NewGraphEnvironment/break/blob/main/DESCRIPTION)

Irvine A (2026). *break: Interactive Watershed Break Point Delineation*.
R package version 0.1.0, <https://github.com/NewGraphEnvironment/break>.

    @Manual{,
      title = {break: Interactive Watershed Break Point Delineation},
      author = {Al Irvine},
      year = {2026},
      note = {R package version 0.1.0},
      url = {https://github.com/NewGraphEnvironment/break},
    }
